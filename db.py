...
# طلبات
# ---------------------------------------------------------------------------
def create_order(user_id, username, full_name, course_id, payment_method) -> int:
    conn = get_conn()
    cur = conn.execute(
        "INSERT INTO orders (user_id, username, full_name, course_id, payment_method, status, created_at) "
        "VALUES (?, ?, ?, ?, ?, 'pending', ?)",
        (user_id, username, full_name, course_id, payment_method, datetime.utcnow().isoformat()),
    )
    conn.commit()
